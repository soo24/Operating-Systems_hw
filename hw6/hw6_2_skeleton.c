/*
	...
	Hints)
		...
		1. 디스플레이 철학자들()을 상호 배타적으로 수정
			1a. mutex를 전역 변수로 선언하고 초기화합니다.
			1b. 디스플레이 철학자의 진입 및 퇴출 섹션을 구현합니다()

		2. 교과서나 강의 슬라이드에 나와 있는 해결책을 따라 철학자들을 동기화합니다.
			2a. 상태 및 젓가락 초기화
			2b. StreadParam을 통해 상태와 젓가락을 나사산에 전달합니다.
			2c. 스레드 기능의 입력 및 종료 섹션을 구현합니다.
			  * 교재 내 용액을 확장하여 '원형 대기' 상태를 방지합니다.
				- 짝수 철학자들이 먼저 왼쪽 젓가락을 집는다.
				- 홀수 철학자들이 먼저 올바른 젓가락을 집는다.

		3. 나사산이 끝나면 모든 뮤텍스를 파괴합니다.
